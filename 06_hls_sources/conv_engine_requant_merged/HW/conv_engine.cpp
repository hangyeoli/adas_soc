...
#pragma HLS LOOP_TRIPCOUNT min=1 max=MAX_IN_CH
        ...
        SHIFT_WINDOW:
            ...
            //
            // Split into 4 separate pipelined loops (row-shift, tail-fill,
            // line_buf-shift, line_buf-tail) rather than one merged loop,
            // deliberately kept in the SAME relative order as the original
            // code's own four pieces, so each piece reads/writes exactly
            // what the original did and correctness follows from the
            // original's own already-verified per-ic reasoning rather than
            // from analyzing a new merged ordering. In particular: ALL row-
            // shift writes (reading each row's OLD tail element) complete
            // before ANY tail-fill write (which overwrites that same tail
            // element) - across every `ic`, not just within one - because
            // WINDOW_ROW_SHIFT is a separate, entirely-preceding loop nest
            // from WINDOW_TAIL_FILL, and C-sim's plain top-to-bottom
            // execution order enforces that regardless of how the pragmas
            // below ask Vitis to schedule the RTL. Likewise WINDOW_TAIL_FILL
            // (reads line_buf's OLD rows) fully precedes LINE_BUF_ROW_SHIFT/
            // LINE_BUF_TAIL_FILL (which overwrite line_buf) - same relative
            // order the original had.
            //
            // NOT SYNTHESIS-CONFIRMED AT THE TIME THIS WAS WRITTEN - only
            // g++-against-standalone-ap_int.h C-sim (all 12 configs + all 13
            // real layers, bit-exact) has verified this matches the
            // original. Before trusting this delivered a real win: re-run
            // csynth and confirm the estimated clock is still under the
            // 5.00 ns target (not just that LUT/cycle numbers look good),
            // then re-run layer 9's cosim and compare against the
            // 1,291,969-cycle baseline.
...
#pragma HLS UNROLL
        ...
        WINDOW_TAIL_FILL:
            for (unsigned ky = 0; ky < k; ky++) {
#pragma HLS LOOP_TRIPCOUNT min=1 max=MAX_K
            WINDOW_TAIL_FILL_STEP:
                for (unsigned ic_step = 0; ic_step < ic_steps_sw; ic_step++) {
#pragma HLS PIPELINE II=1
#pragma HLS LOOP_TRIPCOUNT min=1 max=MAX_IN_CH/TR
                WINDOW_TAIL_FILL_LANE:
                    for (unsigned t = 0; t < TR; t++) {
#pragma HLS UNROLL
                        unsigned ic = ic_step * TR + t;
                        if (ic < ic_count) {
                            window[ic][ky][k - 1] = (ky < k - 1) ? line_buf[ic][ky][c] : px[ic];
                        }
                    }
                }
            }

            // BUG FIX (found in self-review, not by running the tool):
            // conv_layer1.cpp's original `for (ky=0; ky<K-2; ky++)` is
            // only safe there because K=3 is a compile-time constant
            // (K-2=1, always non-negative). Here k is a runtime value
            // that config C (conv_engine_tb.cpp) deliberately sets to 1
            // - unguarded, `k - 2` on an *unsigned* k=1 underflows to a
            // huge value instead of going negative, turning this into an
            // effectively-unbounded loop that indexes line_buf far out
            // of its actual bound. Guarding the whole block (loop AND
            // the write that follows it) on k>=2 is required, not just
            // the write - a k=1 layer needs zero line-buffer rows at
            // all, so skipping the entire block is the correct
            // behavior, not a workaround.
            if (k >= 2) {
            LINE_BUF_ROW_SHIFT:
                for (unsigned ky = 0; ky < k - 2; ky++) {
#pragma HLS LOOP_TRIPCOUNT min=0 max=MAX_K-2
                LINE_BUF_ROW_SHIFT_STEP:
                    for (unsigned ic_step = 0; ic_step < ic_steps_sw; ic_step++) {
#pragma HLS PIPELINE II=1
#pragma HLS LOOP_TRIPCOUNT min=1 max=MAX_IN_CH/TR
                    LINE_BUF_ROW_SHIFT_LANE:
                        for (unsigned t = 0; t < TR; t++) {
#pragma HLS UNROLL
                            unsigned ic = ic_step * TR + t;
                            if (ic < ic_count) {
