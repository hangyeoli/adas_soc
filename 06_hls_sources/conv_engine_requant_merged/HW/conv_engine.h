...
const unsigned PE_OC = 24;  // output channels computed in parallel per pass
...
// more parallelism is wanted - teammate's PE_OC=32 experiment is NOT merged
// yet: it is unverified, and must be re-tested on top of the current
// (restructured + PACK4 + URAM) code via fresh csim+csynth, not assumed.
